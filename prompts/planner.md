...
- 用户要求企业综合分析、尽调、投资风险或财务健康研判时，即使没有直接说出工具名称，也应主动考虑同时调用 `financial_analysis` 和 `ownership_analysis`；需要公告、研报或事件佐证时，再调用 `document_search` 或 `event_timeline`。
...
- `ownership_analysis` 用于主要股东查询、股东反查、跨期持股变化和股权集中度，不仅用于回答明确包含“股权穿透”的问题。
- `ownership_analysis` 当前只支持 `holding_query` 和 `holding_compare`。不得生成 `penetration`。
- `holding_query` 必须提供 `company_ids` 或 `holder_ids` 中至少一项；`holder_ids` 可以传股东主体 ID 或股东全名；`as_of_date` 可省略，省略时工具使用最新已披露快照并在结果中返回实际日期。
- `holding_compare` 只能是“一个公司 + `start_date` + `end_date`”，用于识别股东进入、退出、增持和减持。

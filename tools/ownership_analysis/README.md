...
| `penetration` | 暂未实现 | - | 显式返回 `UNSUPPORTED_QUERY` |
...
| 场景 | 错误类型 |
|---|---|
| `penetration` 或其他未知 operation | `UNSUPPORTED_QUERY` |
...
- `evidence.py`：持股证据组装
- `graph.py` / `sample_data.py`：有界图搜索算法与样例图，保留供未来 `penetration` operation 复用

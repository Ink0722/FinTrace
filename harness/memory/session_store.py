    ...
    def load(self, session_id: str) -> dict:
        with sqlite3.connect(self.path) as connection:
            ...
            row = connection.execute(
                "SELECT current_context, conversation_summary, verified_findings, recent_messages "
                "FROM sessions WHERE session_id = ?",
                [session_id],
            ).fetchone()
        if row is None:
            return {"current_context": {}, "conversation_summary": "", "verified_findings": [], "recent_messages": []}
        return {
    ...
    def save(
        ...
        *,
        ...
        with sqlite3.connect(self.path) as connection:
            connection.execute(
                """
                INSERT INTO sessions (session_id, current_context, conversation_summary, verified_findings, recent_messages, updated_at)
                VALUES (?, ?, ?, ?, ?, ?)

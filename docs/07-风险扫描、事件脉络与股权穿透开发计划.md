...
当前真实事件索引包含 7,311 条标题级事件。股东 v2 索引包含 646,442 条持股记录、94,769 个股东主体、3,438 个上市公司主体和 139 条唯一可审计公司桥接。真实两跳冒烟路径执行耗时约 1.65 秒。上述统计以各索引 `manifest.json` 为最终准据。
...
`risk_scan` 在连续、可比的财务期间内执行已登记的跨科目勾稽规则，返回风险信号、计算输入、公式、阈值、严重度和证据。首版不输出黑箱总分，不使用 LLM 判断规则是否命中，也不宣称识别出财务造假。
...
- `report_periods`：至少两个同口径期间，建议 3 至 5 个年度或相同季度类型；
...
```json
{
  ...
  "rules_skipped": [],
  "coverage": {},
  "rule_version": "financial-risk-rules-v1"
}
```

规则状态至少区分 `triggered`、`not_triggered` 和 `insufficient_data`。不能只返回已命中的规则，否则无法区分“已检查未命中”和“根本没有检查”。`rules_skipped` 必须给出缺失指标、期间不可比或截止日过滤等原因。
...
3. 实现批量取数和期间可比性校验；
4. 以纯函数实现规则并建立人工可计算的小样本金标；
5. 组装 `signals/coverage/evidence`，接入 `interface.py`；
6. 将 capability 标记为已实现，更新 Planner Prompt、CLI 展示和工具 README；
...
当前代码已有 `event_query`、`event_cluster` 所需的筛选、排序、轻量聚类和证据模型，但仍偏样例型实现：数据源主要面向单一 CSV，存在 sample 回退，operation 分发不够严格，正式比赛公告及其他派生事件尚未形成冻结索引。
...
基于已经筛选出的公司和时间范围聚合相关事件，返回事件簇及成员节点。聚类依据应至少包含：事件类型一致/兼容、时间距离、实体重合、规范标题相似度。每个簇保留全部成员事件，不生成“导致”“引发”等因果标签。

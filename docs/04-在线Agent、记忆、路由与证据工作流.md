...
- **Bounded Investigation**：Planner 每轮只产生一个 `AgentAction`，执行和审查后决定继续调查或回答；受 `max_steps`、`max_total_tool_calls`、无新增证据、不可重试错误及文档搜索次数等条件约束，不会无限 ReAct。仅输入唯一公司名称时使用固定的低成本概览队列，不重复调用 Planner 和 Evidence Reviewer 来选择已确定的动作。
- **Clarification / Refusal**：缺少唯一必要条件时澄清，超出数据与能力边界时拒绝；两者均不调用无关工具。主力资金、主力控盘/控仓/增仓、资金流向、龙虎榜、大宗交易、金叉、横盘、量价、筹码、换手率、动态估值和实时成交等高置信度行情或技术形态请求由规则直接识别，不再为确认已知能力边界调用请求解析模型。“营业成本”“经营走势”“业务形态”等财务或经营语境不受该规则影响；混合请求继续执行可支持部分并披露行情能力缺口。
- **Structured Error**：最终回答模型失败时进入结构化错误节点，不使用确定性模板冒充模型回答。

Final Answer Skill 除结构化 Schema 校验外，还检查模型结束原因、回答是否完整收束、Evidence ID 是否属于本轮证据，以及部分回答是否披露系统提供的限制。`finish_reason=length` 或正文停在半句话时自动重试一次；仍未通过时返回结构化错误。只有通过校验的正文才发送给 CLI/GUI。每次 LLM 调用记录总耗时、尝试次数、结束原因、Token 用量、响应字符数和失败原因，供评测区分模型截断、网络失败与输出契约错误。

工具返回 `DATA_NOT_AVAILABLE` 且没有产生新证据时，状态机直接把它作为空检索结果处理，不再调用 Evidence Reviewer 审查空内容；后续由剩余能力和无新增证据预算决定继续调查或收束。

Request Parser 的 `raw_query` 由运行时用户输入确定。Prompt 要求模型原样返回该字段，Schema 校验前程序仍会用运行时值覆盖模型输出，防止遗漏或改写原问题造成无意义重试。其余语义字段继续接受结构化校验。

### 当前验证基线

| 验证层 | 输入或命令 | 结果 |
| --- | --- | --- |
| 完整自动化回归 | `F:\conda_envs\FinTrace\python.exe -m pytest -q --basetemp runtime\pytest-release` | `281 passed`（2026-08-26） |
...
近期消息同时保存用户问题和 Agent 最终回答，常规上限为 12 条。每 6 轮、窗口超过 12 条或字符数超过阈值时，`memory_summarizer` 将即将移出窗口的消息与旧摘要合并；摘要通常控制在 600 至 1000 个中文字符，硬上限为 2000 个字符。摘要成功后保留最近 8 条，调用失败时保留最近 12 条并记录警告，不影响已经生成的本轮回答。如果本轮已有其他模型调用失败，系统延后摘要，避免在收尾阶段重复等待故障接口。能力不支持和需要澄清的轮次保留在近期消息中，但不会为这类非实质性回答单独触发摘要模型。明确切换公司、任务或业务领域时，旧话题待办不会继续进入当前 `open_questions`。摘要 Skill 复用 `QWEN_PLANNER_*` 配置。

`current_context` 保存公司、人物、比较对象、日期、指标和任务，支持唯一主体下的代词继承。本轮明确主体和任务优先于历史上下文：明确公司会替换活动公司；“它、该公司、这家公司”等可唯一确定的指代可以继承活动公司；明确提出新的行业、产业链、上下游、人物或全市场主题时，会结束旧公司的活动状态。工具动作中的上市公司代码必须来自本轮 `ParsedRequest.entities`；历史上下文只有在请求解析器已经完成指代继承后才能使用，Planner 不能直接取上一轮公司替代本轮未解析的新主体。

`verified_findings` 由程序直接从 Evidence Ledger 提取，而不是从最终回答反向抽取。每项保存公司、主题、精简事实、Evidence ID、来源工具和来源轮次；没有 Evidence ID 的内容、`support_level=weak` 的弱证据、模型猜测和失败结果不能写入。每个 Session 最多保留 100 项，Planner 按公司精确匹配并优先选择同任务主题的最多 8 项作为 `memory_hints`。
...
会话保存采用按轮提交的语义。只有正常终止的回答、澄清、能力边界说明或证据不足结果才更新上下文、摘要、已验证发现和轮次；`failed` 或 `llm_failed` 轮仍保留 Trace，但不写入会话记忆。修复外部故障后重试时，不会继承失败轮产生的不完整状态。
...
主体识别区分“未解析的疑似公司”和“无公司主体的行业主题”。前者保留原始名称并返回主体解析缺口，不能由历史公司或默认代码替代；后者以行业、板块、产业链、上下游、赛道或市场主题作为检索条件，允许在没有证券代码时进入文档检索。例如，“某未收录公司近期风险”属于主体未解析，“白酒行业监管趋势”属于行业主题。当前问题明确出现的新主体始终高于会话历史主体。进入 Planner、Reviewer、Repair 和 Final Answer 的 `resolved_context` 会进一步按本轮 `ParsedRequest.entities` 限定，避免会话中保留的旧公司进入本轮工具参数或答案。

任务细分在请求解析阶段完成，但不直接选择工具：财务报告、季报、半年报和公告原文归入公告文档检索；研究报告、机构研究和研报原文归入研报检索；公司近期动态、最新消息和重要事件归入事件调查；近期走势、股价表现和跑赢大盘属于实时行情边界；“能买吗”等确定性投资建议进入预测能力边界。这样可以在进入 Planner 前先确定信息类型，避免用财务、股权或宽泛文档工具代替不相干能力。

## 工具规划与校验

简单、参数完整、操作唯一的事实请求走确定性 Direct Gate。其余请求进入有界调查循环：每轮只规划一个 `AgentAction`，包含工具、操作、参数、目的和预期证据。动作校验器负责能力状态、参数、实体、时间、截止日、重复调用、预算和操作组合，并校验工具主体是否属于本轮请求；失败时仅允许一次最小修复或重新规划。

动作参数采用 operation 级统一预检。系统从工具接口注册表取得对应 Pydantic Schema，在 Action Validator 中注入由 Workflow 管理的 operation 与知识截止日后完成类型转换、必填字段检查、枚举检查和未知字段拒绝；修复后的动作必须重新通过同一 Schema。`execute_one_tool` 在调用接口前再次执行预检，防止节点间状态变化绕过校验。Planner 看到的 Tool Schema、动作校验使用的 Schema 与工具接口模型来自同一组定义，因此参数约束不会在 Prompt、路由和执行层各自维护一份。
...
Evidence Ledger 将每个工具结果规范为带 `evidence_id` 的事实单元，记录来源、对象、时间、原始定位、计算输入/公式、质量标记和限制。进入 LLM Runtime Context 时最多保留 12 条 Evidence，其中 `document_chunk` 最多 4 条，避免长文本检索结果挤占财务、事件、股权和研报观点等结构化证据。Evidence Reviewer 检查每个用户子问题是否已有充分且相互一致的证据；若否，明确证据缺口并在预算内继续调查或降级回答。

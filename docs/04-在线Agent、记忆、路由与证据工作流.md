...
近期消息同时保存用户问题和 Agent 最终回答，常规上限为 12 条。每 6 轮、窗口超过 12 条或字符数超过阈值时，`memory_summarizer` 将即将移出窗口的消息与旧摘要合并；摘要通常控制在 600 至 1000 个中文字符，硬上限为 2000 个字符。摘要成功后保留最近 8 条，调用失败时保留最近 12 条并记录警告，不影响已经生成的本轮回答。如果本轮已有其他模型调用失败，系统延后摘要，避免在收尾阶段重复等待故障接口。明确切换公司、任务或业务领域时，旧话题待办不会继续进入当前 `open_questions`。摘要 Skill 复用 `QWEN_PLANNER_*` 配置。

`current_context` 保存公司、人物、比较对象、日期、指标和任务，支持唯一主体下的代词继承。本轮明确主体和任务优先于历史上下文，不使用机械的新话题清空提示。工具动作中的上市公司代码必须来自本轮 `ParsedRequest.entities`；历史上下文只有在请求解析器已经完成指代继承后才能使用，Planner 不能直接取上一轮公司替代本轮未解析的新主体。
...
主体识别区分“未解析的疑似公司”和“无公司主体的行业主题”。前者保留原始名称并返回主体解析缺口，不能由历史公司或默认代码替代；后者以行业、板块、产业链、赛道或市场主题作为检索条件，允许在没有证券代码时进入文档检索。例如，“某未收录公司近期风险”属于主体未解析，“白酒行业监管趋势”属于行业主题。当前问题明确出现的新主体始终高于会话历史主体。

...
该 Prompt 只负责全局“宪法”。以下内容不要写入本文件，而应由对应 Skill 或 Runtime 动态注入：
...
自然语言规则使用中文；以下工程协议保持英文：`prompt_id`、Schema、JSON key、Pydantic class、Capability、Tool、Operation、Error Code 和状态枚举。